...
@section('title', 'Transparansi Keuangan - Desa Gunung Sembung')

@section('content')

<header class="relative py-[80px] mb-[40px] text-center text-white bg-[url('https://cdn.pixabay.com/photo/2017/09/07/08/54/money-2724241_1280.jpg')] bg-cover bg-center">
    <div class="absolute inset-0 bg-[#0F172A]/85 mix-blend-multiply"></div> 

    <div class="container relative z-10 mx-auto px-5 max-w-6xl">
        <h1 class="m-0 mb-[15px] text-[30px] md:text-[40px] font-bold">Transparansi Keuangan Desa</h1>
        <p class="text-[18px] max-w-[700px] mx-auto opacity-90">
            Akses dan unduh dokumen Anggaran Pendapatan dan Belanja Desa (APBDes) serta laporan pertanggungjawaban secara terbuka.
        </p>
    </div>
</header>
...
<main class="container mx-auto px-5 max-w-6xl mb-[80px]">
    ...
    <div class="flex justify-center flex-wrap gap-[15px] mb-[40px]">
        ...
        <button data-filter="APBDes" class="filter-btn bg-white text-text-muted border border-text-muted px-[25px] py-[10px] rounded-full cursor-pointer font-semibold transition-all duration-300 hover:bg-primary hover:text-white hover:border-primary">
            APBDes
        </button>
        <button data-filter="Realisasi" class="filter-btn bg-white text-text-muted border border-text-muted px-[25px] py-[10px] rounded-full cursor-pointer font-semibold transition-all duration-300 hover:bg-primary hover:text-white hover:border-primary">
            Laporan Realisasi
        </button>
        <button data-filter="LKPJ" class="filter-btn bg-white text-text-muted border border-text-muted px-[25px] py-[10px] rounded-full cursor-pointer font-semibold transition-all duration-300 hover:bg-primary hover:text-white hover:border-primary">
            LKPJ
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[25px]">
        ...
        <div class="col-span-full py-[60px] text-center bg-gray-50 rounded-[16px] border border-dashed border-border">
            ...
            <h3 class="text-[18px] font-bold text-text-main mb-[5px]">Belum Ada Laporan Keuangan</h3>
            <p class="text-text-muted text-[14px]">Dokumen transparansi keuangan akan segera dipublikasikan di sini.</p>
        </div>
        @endforelse
    </div>
