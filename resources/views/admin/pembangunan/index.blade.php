...
@section('title', 'Dokumen Perencanaan Pembangunan - SID Gunung Sembung')
...
<div class="flex flex-col gap-[25px]">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-[15px]">
        <div>
            <h1 class="text-[24px] font-bold text-text-main">Dokumen Perencanaan Pembangunan</h1>
            <p class="text-text-muted text-[14px]">Kelola dokumen RPJMDes dan RKPDes dalam format PDF.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[20px]">
        ...
        <div class="bg-white border border-border rounded-[12px] p-[20px] shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
            ...
            <div class="mt-[20px] pt-[15px] border-t border-border flex justify-between items-center">
                ...
                <form action="{{ route('admin.pembangunan.destroy', $dokumen->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus dokumen ini secara permanen?');">
                    @csrf
                    @method('DELETE')
        ...
        <div class="col-span-full bg-gray-50 border border-border border-dashed rounded-[12px] p-[50px] text-center">
            ...
            <h3 class="text-[16px] font-bold text-text-main mb-[5px]">Belum Ada Dokumen Perencanaan</h3>
            <p class="text-text-muted text-[14px]">Klik tombol "Unggah Dokumen Baru" di sudut kanan atas untuk menambahkan file PDF.</p>
        </div>
        @endforelse
    </div>
</div>
